// moved from src/pages to src/features for Next.js
﻿import React, { useEffect, useMemo, useRef, useState } from 'react'
import { connectEMSC, EmscProps, fromGenericExternal } from '../lib/socketService'
import { LAST_EVENT_KEY } from '../lib/emsc.shared'
import { chan, loadSettings, BOXES, WS_ENABLED_KEY } from '../lib/config'
// Next.js: avoid react-router; provide minimal location shim for query params
function useLocation(): { search: string } {
...
/** Build an absolute asset URL for Next/public */
function asset(url: string) {
  try {
    if (!url) return url as any
    if (/^https?:\/\//.test(url)) return url
    // ensure leading slash for files under public/
