...
function useAdminAuth() {
  ...
  const login = (u: string, p: string) => {
    ...
    const user = env.NEXT_PUBLIC_ADMIN_USER || 'admin'
    const pass = env.NEXT_PUBLIC_ADMIN_PASS || 'change-me'
    if (u === user && p === pass) {
...
export default function Admin() {
  ...
  const envDefault = useMemo(() => {
    const env: any = (typeof process !== 'undefined' && (process as any).env) ? (process as any).env : {};
    return env.NEXT_PUBLIC_EMSC_WS_URL || 'wss://www.seismicportal.eu/standing_order/websocket'
  }, [])
